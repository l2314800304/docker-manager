...
public class LogWebSocketHandler extends TextWebSocketHandler {
    ...
    public LogWebSocketHandler(ContainerLogStreamer logStreamer, ObjectMapper objectMapper) {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        String history = logStreamer.getHistoryLogs(containerId, 100, null);
        if (!history.isBlank()) {
        ...
        // Start streaming
        String streamId = logStreamer.streamLogs(containerId,
                json -> {
                    try {
                        if (session.isOpen()) {
                            session.sendMessage(new TextMessage(json));
