 ...
 * 所有其他 Docker 相关桥接组件（{@link ComposeProjectBridge}、{@link LogStreamBridge} 等）
 * 都通过此组件访问 Docker daemon。</p>
 *
 * <h3>功能分组：</h3>
 * <ul>
 ...
 *   <li><b>日志</b>：获取日志流命令（供 {@link LogStreamBridge} 使用）</li>
 *   <li><b>Stats</b>：获取资源统计命令（供 {@link StatsBridge} 使用）</li>
 *   <li><b>文件系统</b>：从容器复制归档文件（tar 格式）</li>
 *   <li><b>Exec</b>：在容器内执行命令（用于文件系统浏览的 ls/cat 操作）</li>
 * </ul>
 */
@Component
public class DockerClientBridge {

    private static final Logger log = LoggerFactory.getLogger(DockerClientBridge.class);
    private final DockerClient dockerClient;

    public DockerClientBridge(DockerClient dockerClient) {
    ...
    /**
     * 创建日志容器命令（供 {@link LogStreamBridge} 配置和执行）。
     *
    ...
    /**
     * 创建资源统计命令（供 {@link StatsBridge} 配置和执行）。
     *
     ...
     * 这是 {@link FileSystemBridge} 的 tar 降级方案的基础。</p>
     *
     * @param containerId  容器 ID
