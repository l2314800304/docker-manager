...
public class LifecycleBridge {
    ...
    private static final Logger log = LoggerFactory.getLogger(LifecycleBridge.class);
    ...
    public LifecycleBridge(DockerClientBridge dockerClient) {
    ...
    public String updateServiceImage(String projectName, String serviceName,
        ...
        new Thread(() -> {
            try {
                ...
                dockerClient.getDockerClient().pullImageCmd(image)
                        .withTag(newTag)
                ...
                if (!success) {
                    ...
                    var containers = dockerClient.listContainersByLabel("com.docker.compose.project");
     ...
     *
     * @param taskId 任务 ID
     * @return 任务状态映射（status + message），不存在时返回 null
     */
    public Map<String, String> getTaskStatus(String taskId) {
        TaskStatus ts = tasks.get(taskId);
        if (ts == null) return null;
        return Map.of("status", ts.status(), "message", ts.message());
    }

    /**
     * 查询 Docker Hub API 获取镜像的可用 Tag 列表。
     *
     * <p>查询失败时降级为本地镜像 Tag 列表，本地也无法获取时返回常见 Tag。</p>
     *
     * @param imageName 镜像名称（如 "nginx"、"myregistry/app"）
     * @return Tag 名称列表
     */
    ...
    /**
     * 获取本地 Docker 镜像的 Tag 列表。
     *
     * @param imageName 镜像名称
     * @return Tag 列表（当前为常见 Tag 的回退值）
     */
